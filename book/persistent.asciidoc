...
NOTE: This chapter currently covers persistent version 1.3. It will be updated to
cover persistent version 2.0 in the next iteration of the book (covering Yesod 1.4).
...
Since the +Id+ suffix on a type is used to indicate a foreign key relationship
to Persistent, you currently cannot use non-key types that end in +Id+. A
simple workaround is to define a type synonym with a different suffix, e.g.:
