...
Fortunately, +xml-conduit+ provides +ToHtml+ instances for ++Node++s,
++Document++s, and ++Element++s which respect these discrepancies. So by just
using +toHtml+, we can get the correct output.
