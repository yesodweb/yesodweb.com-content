...
period of inactivity. Sites not configured for ssl-only using the
+sslOnlySessions+ and +sslOnlyMiddleware+ functions are vulnerable to +AuthId+
hijacking. Additionally, since there is no server-side component to sessions,
...
sessions, to provide a forced logout capability, if this is desired.
