...
from a config file on launch. Additionally, it load different settings for
developing, testing, staging, and production builds, so you can easily test on
one domain- like localhost- and serve from a different domain. You can modify
these values from the config file.
...
shouldLog:: Determines if a given log message (with associated source and
